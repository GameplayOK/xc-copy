...
if [ -z "${base_system_requirements}" ]; then
	printf '%s\n' "> Unsupported system [${platform}]. Exiting..."
	exit 127
fi

printf '%s\n' "> Install dev tools and system packages? (y/n)"
printf '%s' '[commitizen pipx pre-commit shellcheck shfmt shellspec] '
stty raw -echo
answer=$(head -c 1)
stty "${old_stty_cfg}"
...
if echo "${answer}" | grep -iq "^y"; then
	...
	# Additonal packages that are also required in CI environments
